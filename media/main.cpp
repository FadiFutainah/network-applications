...
// FFT
//void RotateCamera(float a1, float a2, float a3);
...
float speed=0.5f;  //سرعة حركة الكاميرا
...
// Sky Box 
int top, down, Left, Right, back, front;

// Textsure for the land
...
int toRadian(float angle)
{
	return (angle / 180.0)*3.14f;
} 

int fftup = 0, fftdown = 0;

float cam_rad = 360.0f;

int cam_ax = 0, cam_ay = 0;

POINT mouse_pos;
float fftspeed = 8;
bool main_menu = false; 

float yvar = 2.5, xvar = 0, zvar = 0;

#define FRAMES 60

bool isCollided(float x1, float x2, float y1, float y2, float z1, float z2, float x, float y, float z){
	return (x >= x1 && x <= x2) && (y >= y1 && y <= y2) && (z >= z1 && z <= z2);
}

bool isCollidedWithWall(float x ,float y,float z){
	return isCollided(-10, 10, 0, 0, -10, 10, xvar, yvar, zvar);
}

bool isCollidedAll(float x ,float y,float z){
	return isCollidedWithWall(x, y, z);
}
...
void camera() {	
	...
	//	yvar += fftspeed;
	//	/*if(isCollidedAll(xvar, yvar, zvar)){
	//		yvar -= fftspeed;
	//	}*/
	//} 
	...
	//} 
	if (keys['S']){
		if(isCollidedWithWall(xvar,yvar,zvar))
		zvar += cos(toRadian(cam_ax)) * 60 / FRAMES * fftspeed; 
		xvar += sin(toRadian(cam_ax)) * 60 / FRAMES * fftspeed;
		cout << xvar << endl;
		{
			zvar -= cos(toRadian(cam_ax)) * 60 / FRAMES * speed;
			xvar -= sin(toRadian(cam_ax)) * 60 / FRAMES * speed;
			
		}
		
	} 
	if (keys['W']) {
			float z1 = zvar + cos(toRadian(cam_ax)) * 60 / FRAMES * speed; 
			float x1 = xvar + sin(toRadian(cam_ax)) * 60 / FRAMES * speed;
		if(!isCollidedWithWall(x1,yvar,z1)){
		zvar = z1;
		xvar = x1;
/*
	...
	if (keys['A']){
		if(isCollidedWithWall(xvar,yvar,zvar))
		zvar += sin(toRadian(cam_ax)) * 60 / FRAMES * fftspeed; 
		xvar -= cos(toRadian(cam_ax)) * 60 / FRAMES * fftspeed;
		{
			zvar -= sin(toRadian(cam_ax)) * 60 / FRAMES * speed; 
			xvar += cos(toRadian(cam_ax)) * 60 / FRAMES * speed;
	...
	//if (keys[VK_ESCAPE]) main_menu = !main_menu;
	...
	RotateCamera(cam_ax, cam_ay, 0);
	MoveCamera(xvar, yvar, zvar);
}
...
*/
	...
	asphalt = LoadTexture("Data\\images\\Asphalt.bmp"); // السكة
	...
	border = LoadTexture("Data\\images\\border.bmp"); // الزريعة تبع الشجرة
...
//تابع رسم المقطورة 2
void drawTrain2(double tx,double y,double z,double scale)
{
...
//تابع رسم المقطورة3
void drawTrain3(double tx,double y,double z,double scale)
{
...
//تابع رسم المقطورة 4
void drawTrain4(double tx,double y,double z,double scale)
{
...
void draw_glasses(float tx)
{
	...
	//الزجاج الأول اليميني 2
		glPushMatrix();
		glTranslated(-800+tx,-160,125);
	...
	//الزجاج الأول اليميني 3
		glPushMatrix();
		glTranslated(-1350+tx,-160,125);
	...
	//الزجاج الأول اليميني4
		glPushMatrix();
		glTranslated(-1875+tx,-160,125);
	...
	//2الزجاج الأول اليساري
		glPushMatrix();
		glTranslated(-800+tx,-160,-150);
	...
	//3الزجاج الأول اليساري
		glPushMatrix();
		glTranslated(-1350+tx,-160,-150);
	...
	//4الزجاج الأول اليساري
		glPushMatrix();
		glTranslated(-1875+tx,-160,-150);
	...
	//الزجاج الثاني اليميني2
		glPushMatrix();
		glTranslated(-975+tx,-160,125);
	...
	//الزجاج الثاني اليميني3
		glPushMatrix();
		glTranslated(-1525+tx,-160,125);
	...
	//الزجاج الثاني اليميني4
		glPushMatrix();
		glTranslated(-2050+tx,-160,125);
	...
	//الزجاج الثاني اليساري2
		glPushMatrix();
		glTranslated(-975+tx,-160,-150);
	...
	//الزجاج الثاني اليساري3
		glPushMatrix();
		glTranslated(-1525+tx,-160,-150);
	...
	//الزجاج الثاني اليساري4
		glPushMatrix();
		glTranslated(-2050+tx,-160,-150);
...
if(counter<4000)
 ...
 if(counter>4000)
 {
   ...
   //تابع تلقي إشارات الكيبورد
	...
	//FFT	 
	 ...
	 if(movex>50000)
	 {
		...
		glColor4f(0,0,0,0.3);
		//رسم ظل المقطورة2
		drawTrain(-850+movex,-200,movez,30);
		
		glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		glEnable(GL_BLEND);
		glColor4f(0,0,0,0.3);
		//رسم ظل المقطورة3
		drawTrain(-1400+movex,-200,movez,30);
		
		glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		glEnable(GL_BLEND);
		glColor4f(0,0,0,0.3);
		//رسم ظل المقطورة4
		drawTrain(-1920+movex,-200,movez,30);
   ...
   //ظل الاشجار
    ...
    glDisable(GL_LIGHTING);
	glPushMatrix();
    drawTrain2(-850+movex,-200,movez,30);
	glPopMatrix();
	glDisable(GL_LIGHTING);
    ...
    glDisable(GL_LIGHTING);
	glPushMatrix();
    drawTrain3(-1400+movex,-200,movez,30);
	glPopMatrix();
	glDisable(GL_LIGHTING);
    ...
    glDisable(GL_LIGHTING);
	glPushMatrix();
    drawTrain4(-1940+movex,-200,movez,30);
	glPopMatrix();
	glDisable(GL_LIGHTING);
